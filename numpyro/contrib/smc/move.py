...
from .util import DataModel, compute_moments, unconstrain_samples, compute_acceptance_probs
...
class BasicMoveKernel:
    ...
    def make_proposal(self, samples: dict) -> Distribution:
        ...
        """
        latent_mean, latent_covariance = compute_moments(unconstrain_samples(samples, self.model_trace))
        return MultivariateNormal(latent_mean,
                                  covariance_matrix=latent_covariance + self.cov_nugget * jnp.eye(latent_mean.shape[0]))

    def __call__(self, rng_key: jax.random.PRNGKey, model: DataModel, samples: dict) -> dict:
        """
        ...
        if self.model_trace is None:
            init_params, _, self.postprocess_fn, self.model_trace = initialize_model(rng_key, model,
                                                                                     model_kwargs=model.data)
        ...
        new_unconstrained_samples = jax.vmap(self.unpack_tf)(proposal_dist.sample(rng_key_s, (n_samples,)))
        new_samples = self.postprocess_fn(new_unconstrained_samples)

        # Compute Metropolis-Hastings acceptance probabilities
        accept_log_prob = compute_acceptance_probs(samples, new_samples, self.model_trace, model, proposal_dist)
        ...
        if n_accepted > 0:
            ...
            for k in samples.keys():
                result[k] = jnp.concatenate([new_samples[k][accepted], samples[k][jnp.logical_not(accepted)]])
        else:
            result = samples
